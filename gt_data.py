...
def unlabel_default_loader(id, root):
    img = cv2.imread(os.path.join(root, '{}.jpg').format(id))
...
class AugImageFolder(data.Dataset):

    def __init__(self, trainlist, root,mode):
        self.ids = trainlist
        self.loader = default_loader
        self.strongloader=strongloader
        self.root = root
        self.mode=mode
    ...
    def __getitem__(self, index):
        ...
        if self.mode=='strong':
            ...
            mask =Image.open(os.path.join(self.root + '{}.png').format(id), mode='r').convert('L')
